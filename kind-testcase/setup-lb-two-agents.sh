...
kind create cluster --name lb-client-twoagents
kind create cluster --name lb-server-twoagents

echo Deploying Lacework agent on client
kubectl config use-context kind-lb-client-twoagents
kubectl delete namespace lacework
kubectl create namespace lacework
kubectl config set-context --current --namespace lacework
cat lacework-cfg-k8s.yaml | sed "s/insertaccesstoken/$1/" | sed "s/insertkubernetesclusterhere/lb-client-twoagents/" | kubectl apply -f -
...
kubectl create namespace lacework
kubectl config set-context --current --namespace lacework
cat lacework-cfg-k8s.yaml | sed "s/insertaccesstoken/$1/" | sed "s/insertkubernetesclusterhere/lb-server-twoagents/" | kubectl apply -f -
kubectl apply -f lacework-k8s.yaml
kubectl config set-context --current --namespace default
